function [f_min, time_min, theta_min] = hybrid_method(f, n_theta, method)
...
while(cputime <time_0+300)
	time_1=cputime;
    %Simmulated annealing method for 20 seconds
    ...
    %Steepest descendent method for 10 seconds
    C = steepest_descendent(f, C, 10);
    if (f(C)<f_min)
    	f_min = f(C);
    	theta_min = C;
    end
