function [f_min, time_min, theta_min] = adapted_simulated_annealing(f, n_theta, method)
...
while(cputime <time_0+280)
    ...
    %If the result schieved is the minimal of the sequence until the
    %iteration consdered, then the combination of angles is saved as the
    %minimal
    if (E_c<f_min)
        f_min = E_c;
        theta_min = C;
    ...
    %Applying steepest descent for 20 seconds after SA method
C = steepest_descendent(f, C, 20);
if (f(C)<f_min)
    f_min = f(C);
    theta_min = C;
end
